// Getting the list of all items that contain the class .content, there should be 6 of them
...
// Getting the list of all the content items
var contentContent = document.getElementsByClassName('content');
...
// This function finds the content element with an active class, stores a reference to it and sends it to another function
...
function findActiveElement() {
  for(let i = 0; i < contentContent.length; i++) {
    if(contentContent[i].classList.contains(activeClass)) {
      activecontentElement = contentContent[i];
      // We pass the element with an active class to our function that deals with removing the active class.
      applyClickOnCloseButton(activecontentElement);
    }
  }
}

// This function receives the element that contains an active class and removes it.
function applyClickOnCloseButton(activecontentItem) {
  for(let i = 0; i < closeButtons.length; i++) {
    closeButtons[i].addEventListener('click', function() {
      activecontentItem.classList.remove(activeClass);
    });
  }
}
